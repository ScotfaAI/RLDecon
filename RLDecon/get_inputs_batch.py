...
from tkinter import simpledialog, messagebox, filedialog, Listbox
...
class MultipleInputFileDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        self.select_folder_button = tk.Button(self.master, text="Select Folder", command=self.select_folder_and_display_tif_files)
        self.select_folder_button.grid(row=0, column=1, sticky="ew")
        
        # Create a Listbox to display the file paths
        self.file_listbox = Listbox(self.master, width=100, height=20)
    ...
    def select_folder_and_display_tif_files(self):
        folder_path = filedialog.askdirectory()
        if folder_path:  # Ensure the user selected a folder
            # Get all .tif files in the selected folder, with full paths
            tif_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.tif')]
            # Sort the files by name
            tif_files.sort()
            
            self.selected_tif_files = tif_files  # Store the sorted list in the global variable
            
            # Clear the Listbox before adding new entries
            self.file_listbox.delete(0, tk.END)
            
            # Display the sorted list with full paths in the Listbox
...
def get_inputs_batch():
    ...
    while(not valid):
        try:
            ...
            input_files = file_inputs['image_file']
            input_file_str = input_files[0]
